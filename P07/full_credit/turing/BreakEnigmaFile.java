...
public class BreakEnigmaFile {
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 0 || args.length > 2) {
            System.err.println("usage: java BreakEnigmaFile <#lines> [<threads>]");
            System.exit(-1);
        }
    ...
    // Rewrite this section to split the work between numThreads threads!
        ...
        for (int i = 0; i < numThreads; ++i) 
        {
            ...
            threads[i] = new Thread(() -> 
            {
                ...
                for (int j = threadIndex; j < encrypteds.size(); j += numThreads) 
                {
                    BreakEnigmaFile breaker = new BreakEnigmaFile();
                    breaker.breakManager(j);
                }
                System.out.println("### Finished thread " + threadIndex);
            });
            threads[i].start();
        }
    ...
    public void breakManager(int index) 
    {
        EncryptedPair pair = encrypteds.get(index); //NULL;
                                    ...
                                    for (var rs2 : Enigma.positions) {
                                        for (var rs3 : Enigma.positions) {
                                            Setting setting = new Setting(
                                                    new String[]{rotor1, rotor2, rotor3},
                                                    new int[]{rp1, rp2, rp3},
                                                    new int[]{rs1, rs2, rs3});
                                            ...
                                            if (decrypted.hashCode() == hashcode) {
                                                ...
                                                {
                                                    hashCodeSum += decrypted.hashCode();
                                                }
                                                // =======================================
                                                return;
                                            }
...
public class BreakEnigmaFile {
    ...
    // IMPORTANT: ArrayList is NOT thread-safe! If you read or write it within
    //   a thread, you MUST take precautions as discussed in Lecture 14!
    private static final List<EncryptedPair> encrypteds = new ArrayList<>();
